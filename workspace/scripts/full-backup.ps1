...
Write-Host "=== OpenClaw Full Backup ===" -ForegroundColor Green
...
robocopy "C:\Users\Administrator\.openclaw" $BACKUP_DIR /E /XD node_modules .git extensions /R:3 /W:5 /MT:8 | Out-Null
...
# Add all files
git add -A | Out-Null
git commit -m "Full backup ${BACKUP_DATE}" | Out-Null
