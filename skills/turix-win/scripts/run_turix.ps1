...
# ---------- Configuration ----------
$ProjectDir = "C:\Users\Administrator\.openclaw\workspace\turix"
$ConfigFile = Join-Path $ProjectDir "examples\config.json"
$EnvName = "turix_env"
$CondaCmd = "conda"
$RequiredBranch = "multi-agent-windows"
$EnvPython = "C:\Users\Administrator\AppData\Local\Programs\Python\Python312\python.exe"
...
function Preflight-Checks {
    ...
    if (-not $conda) {
        Log-Warn "conda not found in PATH, using EnvPython"
    }
    ...
    Log-Info "Env Python: $EnvPython"
    if ($LASTEXITCODE -ne 0) {
        Log-Warn "Unable to inspect conda environments (conda not in PATH)"
    }
...
function Main {
    ...
    if ($DryRun) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($EnvPython) -and (Test-Path -LiteralPath $EnvPython)) {
            Write-Host "  Command: $EnvPython examples/main.py"
        } else {
