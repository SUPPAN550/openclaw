...
class GraphSetup:
    ...
    def setup_graph(
        self, selected_analysts=["market", "social", "sentiment", "news", "fundamentals", "macro", "smart_money", "industry", "valuation", "financial_quality", "management", "risk", "institutional", "flows", "game_theory"],
        checkpointer=None
    ):
        ...
        if "sentiment" in selected_analysts:
            ...
            tool_nodes["sentiment"] = self.tool_nodes.get("sentiment")
            done_nodes["sentiment"] = analyst_done_node

        if "game_theory" in selected_analysts:
            analyst_nodes["game_theory"] = create_game_theory_analyst(
                self.quick_thinking_llm, self.data_collector
            )
            tool_nodes["game_theory"] = self.tool_nodes.get("game_theory")
            done_nodes["game_theory"] = analyst_done_node
        ...
        if "industry" in selected_analysts:
            ...
            tool_nodes["industry"] = self.tool_nodes.get("industry")
        ...
        if "valuation" in selected_analysts:
            ...
            tool_nodes["valuation"] = self.tool_nodes.get("valuation")
            done_nodes["valuation"] = analyst_done_node

        if "financial_quality" in selected_analysts:
            analyst_nodes["financial_quality"] = create_financial_quality_analyst(
                self.quick_thinking_llm, self.data_collector
            )
            tool_nodes["financial_quality"] = self.tool_nodes.get("financial_quality")
            done_nodes["financial_quality"] = analyst_done_node

        if "management" in selected_analysts:
            analyst_nodes["management"] = create_management_analyst(
                self.quick_thinking_llm, self.data_collector
            )
            tool_nodes["management"] = self.tool_nodes.get("management")
            done_nodes["management"] = analyst_done_node

        if "risk" in selected_analysts:
            analyst_nodes["risk"] = create_risk_analyst(
                self.quick_thinking_llm, self.data_collector
            )
            tool_nodes["risk"] = self.tool_nodes.get("risk")
            done_nodes["risk"] = analyst_done_node

        if "institutional" in selected_analysts:
            analyst_nodes["institutional"] = create_institutional_analyst(
                self.quick_thinking_llm, self.data_collector
            )
            tool_nodes["institutional"] = self.tool_nodes.get("institutional")
            done_nodes["institutional"] = analyst_done_node

        if "flows" in selected_analysts:
            analyst_nodes["flows"] = create_flows_analyst(
                self.quick_thinking_llm, self.data_collector
            )
            tool_nodes["flows"] = self.tool_nodes.get("flows")
            done_nodes["flows"] = analyst_done_node
