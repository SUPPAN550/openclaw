...
class RiskItemSchema(BaseModel):
    name: str = Field(default="", description="风险名称，15字以内")
    level: Literal["high", "medium", "low"] = Field(default="medium", description="风险等级")
    description: str = Field(default="", description="一句话说明，30字以内")


class KeyMetricSchema(BaseModel):
    name: str = Field(default="", description="指标名称，如 PE、ROE、营收增速")
    value: str = Field(default="", description="指标值，包含单位，如 28.5x、15.2%")
    status: Literal["good", "neutral", "bad"] = Field(default="neutral", description="优劣判断")
...
def extract_structured_data(
    ...
    try:
        ...
        # 兼容性处理：如果 LLM 返回的 risks/key_metrics 是列表而非对象数组，尝试转换
        if isinstance(parsed, dict):
            for key in ['risks', 'key_metrics']:
                if key in parsed and isinstance(parsed[key], list):
                    ...
                    for item in parsed[key]:
                        if isinstance(item, dict):
                            ...
                            if key == 'risks':
                                fixed_items.append({
                                    'name': item.get('name', item.get('risk_name', '')),
                                    'level': item.get('level', item.get('risk_level', 'medium')),
                                    'description': item.get('description', item.get('desc', ''))
                                })
                            else:  # key_metrics
                                fixed_items.append({
                                    'name': item.get('name', item.get('metric_name', '')),
                                    'value': item.get('value', item.get('metric_value', '')),
                                    'status': item.get('status', item.get('metric_status', 'neutral'))
                                })
                        else:
                            # 非字典类型的项，创建空对象
                            if key == 'risks':
                                fixed_items.append({'name': '', 'level': 'medium', 'description': ''})
                            else:
                                fixed_items.append({'name': '', 'value': '', 'status': 'neutral'})
                    parsed[key] = fixed_items
