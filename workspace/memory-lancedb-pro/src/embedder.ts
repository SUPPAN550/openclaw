...
export class Embedder {
  ...
  private async embedSingle(text: string, task?: string, depth: number = 0, signal?: AbortSignal): Promise<number[]> {
    ...
    try {
      ...
      // Handle both OpenAI format (data[0].embedding) and MiniMax format (vectors[0])
      let embedding: number[] | undefined;
      if (response.data && response.data[0]?.embedding) {
        embedding = response.data[0].embedding as number[];
      } else if ((response as any).vectors && (response as any).vectors[0]) {
        embedding = (response as any).vectors[0] as number[];
      }
      if (!embedding) {
  ...
  private async embedMany(texts: string[], task?: string): Promise<number[][]> {
    ...
    try {
      ...
      // Fill in embeddings for valid texts - handle both OpenAI and MiniMax formats
      if (response.data) {
      ...
      } else if ((response as any).vectors) {
        // MiniMax format: response.vectors is [[e1, e2, ...], ...]
        (response as any).vectors.forEach((vec: number[], idx: number) => {
          const originalIndex = validIndices[idx];
          this.validateEmbedding(vec);
